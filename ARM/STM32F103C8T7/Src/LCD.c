 ...
 */
/**
 * The function initializes the LCD by configuring the necessary pins and sending commands to set the
 * LCD mode and clear the display.
 */
void LCD_init(void)
{
	/* Configure the direction for RS and E pins as output pins */
	//DIO_SetPinDirection(LCD_RS_PORT_ID, LCD_RS_PIN_ID, PIN_OUTPUT);
	GPIO_SetPinDirection(LCD_RS_PORT_ID, LCD_RS_PIN_ID, OUT_PUSH_PULL, OUTPUT_10MHZ);

	//DIO_SetPinDirection(LCD_E_PORT_ID, LCD_E_PIN_ID, PIN_OUTPUT);
	GPIO_SetPinDirection(LCD_E_PORT_ID, LCD_E_PIN_ID, OUT_PUSH_PULL, OUTPUT_10MHZ);

#if (LCD_DATA_BITS_MODE == _4_BIT_MODE)
	/* Configure 4 pins in the data port as output pins */
	GPIO_SetPinDirection(LCD_DATA_PORT_ID, LCD_DB4_PIN_ID, OUT_PUSH_PULL, OUTPUT_10MHZ);
	GPIO_SetPinDirection(LCD_DATA_PORT_ID, LCD_DB5_PIN_ID, OUT_PUSH_PULL, OUTPUT_10MHZ);
	GPIO_SetPinDirection(LCD_DATA_PORT_ID, LCD_DB6_PIN_ID, OUT_PUSH_PULL, OUTPUT_10MHZ);
	GPIO_SetPinDirection(LCD_DATA_PORT_ID, LCD_DB7_PIN_ID, OUT_PUSH_PULL, OUTPUT_10MHZ);
	...
	//LCD_sendCommand(LCD_TWO_LINES_FOUR_BITS_MODE); //! TODO: remove

#elif (LCD_DATA_BITS_MODE == _8_BIT_MODE)
	/* Configure the data port as output port */
	DIO_SetPortDirection(LCD_DATA_PORT_ID, PORT_OUTPUT);
...
{
	GPIO_SetPinValue(LCD_RS_PORT_ID, LCD_RS_PIN_ID, LOGIC_LOW); /* Instruction Mode RS=0 */
	GPIO_SetPinValue(LCD_E_PORT_ID, LCD_E_PIN_ID, LOGIC_HIGH);	/* Enable LCD E=1 */

#if (LCD_DATA_BITS_MODE == _4_BIT_MODE)
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB4_PIN_ID, READ_BIT(command, 4));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB5_PIN_ID, READ_BIT(command, 5));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB6_PIN_ID, READ_BIT(command, 6));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB7_PIN_ID, READ_BIT(command, 7));

	GPIO_SetPinValue(LCD_E_PORT_ID, LCD_E_PIN_ID, LOGIC_LOW);  /* Disable LCD E=0 */
	GPIO_SetPinValue(LCD_E_PORT_ID, LCD_E_PIN_ID, LOGIC_HIGH); /* Enable LCD E=1 */

	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB4_PIN_ID, READ_BIT(command, 0));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB5_PIN_ID, READ_BIT(command, 1));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB6_PIN_ID, READ_BIT(command, 2));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB7_PIN_ID, READ_BIT(command, 3));
...
{
	if (LCD_NUM_LINES > row && LCD_NUM_POSITIONS > col)
	{
		...
		{
			case 0:
				lcd_memory_address = col + LCD_FIRST_LINE;
				break;
			case 1:
				lcd_memory_address = col + LCD_SECOND_LINE;
				break;
			case 2:
				lcd_memory_address = col + LCD_THIRD_LINE;
				break;
			case 3:
				lcd_memory_address = col + LCD_FOURTH_LINE;
				break;
		}
...
{
	GPIO_SetPinValue(LCD_RS_PORT_ID, LCD_RS_PIN_ID, LOGIC_HIGH); /* Data Mode RS=1 */
	GPIO_SetPinValue(LCD_E_PORT_ID, LCD_E_PIN_ID, LOGIC_HIGH);	 /* Enable LCD E=1 */

#if (LCD_DATA_BITS_MODE == _4_BIT_MODE)
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB4_PIN_ID, READ_BIT(data, PIN4));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB5_PIN_ID, READ_BIT(data, PIN5));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB6_PIN_ID, READ_BIT(data, PIN6));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB7_PIN_ID, READ_BIT(data, PIN7));

	GPIO_SetPinValue(LCD_E_PORT_ID, LCD_E_PIN_ID, LOGIC_LOW);  /* Disable LCD E=0 */
	GPIO_SetPinValue(LCD_E_PORT_ID, LCD_E_PIN_ID, LOGIC_HIGH); /* Enable LCD E=1 */

	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB4_PIN_ID, READ_BIT(data, PIN0));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB5_PIN_ID, READ_BIT(data, PIN1));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB6_PIN_ID, READ_BIT(data, PIN2));
	GPIO_SetPinValue(LCD_DATA_PORT_ID, LCD_DB7_PIN_ID, READ_BIT(data, PIN3));
 ...
 */
void LCD_displayBinary(uint32 num)
{
